import {AUTH_CLEAN_FORM, AUTH_SET_USER, AUTH_SET_AUTH, AUTH_LOGOUT, AUTH_CHANGE_FORM} from '../types/types'
import {appLoadingShow, appLoadingHidden, appAddAlertMessage, appReady} from './appActions'
import {registerApi, loginApi} from '../api/authApi'
import {setLocalStorage, deleteLocalStorage, getDataLocalStorage} from '../services/authStorageServices'

export type ActionsTypes = authChangeFormType | authCleanFormType | authSetUserType | authSetAuthType | authLogoutType

type authChangeFormType = {type: typeof AUTH_CHANGE_FORM, payload: object}
export const authChangeForm = (event: object):authChangeFormType => ({type: AUTH_CHANGE_FORM, payload: event})

type authCleanFormType = {type: typeof AUTH_CLEAN_FORM}
export const authCleanForm = ():authCleanFormType => ({type: AUTH_CLEAN_FORM})

type dataUserType = {id: string, login: string, token: string}
type authSetUserType = {type: typeof AUTH_SET_USER, payload: dataUserType}
export const authSetUser = (data: dataUserType):authSetUserType => ({type: AUTH_SET_USER, payload: data})

type authSetAuthType = {type: typeof AUTH_SET_AUTH}
export const authSetAuth = ():authSetAuthType => ({type: AUTH_SET_AUTH})

type authLogoutType = {type: typeof AUTH_LOGOUT}
export const authLogout = ():authLogoutType => ({type: AUTH_LOGOUT})

export const authSetAuthData = () => {
    const data:dataUserType = getDataLocalStorage()
    return async (dispatch: any) => {
        if (data) {
            await dispatch(authSetUser(data))
            await dispatch(authSetAuth())
        }
        await dispatch(appReady())
    }
}

export const loginHandler = (login: string, password: string) => {
    return async (dispatch: any) => {
        try {
            if (!login || !password) return dispatch(appAddAlertMessage('error', 'Логин и пароль должны быть заполнены'))
            await dispatch(appLoadingShow())
            await loginApi({login, password})
                .then(res => {
                        dispatch(appAddAlertMessage('info', res.data.message))
                        const userData = {id: res.data.id, login: res.data.login, token: res.data.token}
                        dispatch(authSetUser(userData))
                        dispatch(authSetAuth())
                        setLocalStorage(userData)
                        dispatch(authCleanForm())})
                .catch(err => dispatch(appAddAlertMessage('error', err.response.data.message || 'Что-то пошло не так')))
            await dispatch(appLoadingHidden())
        } catch (e) {
            console.log(e)
        }
    }
}

export const logoutHandler = () => {
    return async (dispatch:any) => {
        try {
            await deleteLocalStorage()
            dispatch(authLogout())
        } catch (e) {
            console.log(e)
        }
    }
}

export const registrationHandler = (login: string, password: string) => {
    return async (dispatch:any) => {
        try {
            if (!login || !password) return dispatch(appAddAlertMessage('error', 'Логин и пароль должны быть заполнены'))
            await dispatch(appLoadingShow())
            await registerApi({login, password})
                .then(res => dispatch(appAddAlertMessage('success', res.data.message)))
                .catch(err => dispatch(appAddAlertMessage('error', err.response.data.message || 'Что-то пошло не так!')))
            await dispatch(appLoadingHidden())
        } catch (e) {
            console.log(e)
        }
    }
}